 ...
 */
TEST(wallet_zkeys_tests, store_and_load_sapzkeys) {
    SelectParams(CBaseChainParams::MAIN);

    CWallet wallet;
    if(!wallet.HaveHDSeed())
    {
    ...
    ASSERT_TRUE(wallet.AddSaplingSpendingKey(sk, addr));
    ...
    // manually add new spending key to wallet
    ASSERT_TRUE(wallet.AddSaplingZKey(sk, addr));
    ...
    auto ivk2 = fvk2.in_viewing_key();
    
    libzcash::SaplingExtendedSpendingKey keyOut2;
    wallet.GetSaplingSpendingKey(fvk2, keyOut2);

    ASSERT_TRUE(wallet.LoadSaplingZKey(keyOut2));
    ...
    CKeyMetadata m = wallet.mapSaplingZKeyMetadata[ivk2];
 ...
 */
TEST(wallet_zkeys_tests, write_cryptedzsapkey_direct_to_db) {
    SelectParams(CBaseChainParams::TESTNET);
    ...
    auto paymentAddress = boost::get<libzcash::SaplingPaymentAddress>(address);
    ...
    //ASSERT_TRUE(boost::get<libzcash::SaplingPaymentAddress>(&address2) != nullptr);
    auto paymentAddress2 = boost::get<libzcash::SaplingPaymentAddress>(address2);
    ...
    libzcash::SaplingExtendedSpendingKey keyOut;
    HDSeed hdSeed;
    wallet.GetHDSeed(hdSeed);
    auto m = libzcash::SaplingExtendedSpendingKey::Master(hdSeed);
    auto sk = m.Derive(0);
    ASSERT_TRUE(wallet.AddSaplingZKey(sk, sk.DefaultAddress()));
    ...
    wallet2.GetSaplingSpendingKey(fvk, keyOut);
    //ASSERT_EQ(sk, keyOut);
    //ASSERT_FALSE(paymentAddress == keyOut.address());
